...
public class AItestScript : MonoBehaviour {
	...
	void Update () {
		...
		ai.Search (ai.root);
		Debug.DrawRay ( transform.position, player.transform.position - (Vector3) myVector, Color.red);
	...
	*/
	...
	public bool RangeCheck(){
		if (Vector2.Distance(player.transform.position, myVector) < aggrorange) {
			//Debug.Log ("In range");
			return true;
		}
